...
public class AdvancedVideoEditor: PSVideoEditing {
    ...
    public func edit(video: PSVideo, completion: @escaping (_ video: PSVideo?, _ tempPath: URL?) -> Void) {
        process(video: video, completion: completion)
    }
    
    public func crop(avAsset: AVAsset, completion: @escaping (URL?) -> Void) {
        guard let outputURL = PSVideoEditInfo.outputURL else {
            completion(nil)
            return
        }
        
        guard let writer = try? AVAssetWriter(outputURL: outputURL as URL, fileType: PSVideoEditInfo.file.type),
    ...
    fileprivate func wireVideo(_ avAsset: AVAsset) {
        ...
        if !videoTracks.isEmpty {
            ...
            videoOutput.videoComposition = PSVideoEditInfo.composition(avAsset)
            ...
            let videoInput = AVAssetWriterInput(mediaType: AVMediaType.video,
                                                outputSettings: PSVideoEditInfo.videoSettings,
                                                sourceFormatHint: avAsset.ps_videoDescription)
    ...
    fileprivate func wireAudio(_ avAsset: AVAsset) {
        ...
        if !audioTracks.isEmpty {
            ...
            let audioInput = AVAssetWriterInput(mediaType: AVMediaType.audio,
                                                outputSettings: PSVideoEditInfo.audioSettings,
                                                sourceFormatHint: avAsset.ps_audioDescription)
